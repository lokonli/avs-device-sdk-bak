...
#ifndef ALEXA_CLIENT_SDK_KWD_KITTAI_INCLUDE_NS_NSKEYWORDDETECTOR_H_
#define ALEXA_CLIENT_SDK_KWD_KITTAI_INCLUDE_NS_NSKEYWORDDETECTOR_H_
...
class NSKeyWordDetector : public AbstractKeywordDetector {
public:
    /**
     * The configuration used by the NSKeyWordDetector to set up keywords to be notified of.
     */
    struct NSConfiguration {
     ...
     * @param kittAiConfigurations A vector of @c NSConfiguration objects that will be used to initialize the
     * engine and keywords.
     ...
     * @return A new @c NSKeyWordDetector, or @c nullptr if the operation failed.
     * @see https://github.com/Kitt-AI/snowboy for more information regarding @c audioGain and @c applyFrontEnd.
     */
    static std::unique_ptr<NSKeyWordDetector> create(
...
/*        const std::string& resourceFilePath,
        const std::vector<NSConfiguration> kittAiConfigurations,
        float audioGain,
        bool applyFrontEnd,*/
...
private:
     ...
     * called once with each new @c NSKeyWordDetector.
     *
     * @param audioFormat The format of the audio data located within the stream.
     ...
     */
    bool isAudioFormatCompatibleWithNS(avsCommon::utils::AudioFormat audioFormat);
...
#endif  // ALEXA_CLIENT_SDK_KWD_NS_INCLUDE_NS_NSKEYWORDDETECTOR_H_
